...
import os, sys
from etc import config, constants
...
class Motif_Conjoint_Preprocess(preprocessor_worker):
    ...
    def get_conjoint_triads_fast(self, chunk: pd.DataFrame, col):
        ...
        translation_table = str.maketrans(constants.CONJOINT_TRIADS)
        ...
        vectorizer = CountVectorizer(analyzer='char', ngram_range=(3, 3), vocabulary=constants.TRIAD_NAMES)
        ...
        triad_df = pd.DataFrame(
            ...
            columns=[f"{col}_CTD_{t}" for t in constants.TRIAD_NAMES],
            index=chunk.index
        )
        return triad_df
